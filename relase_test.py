...
def write_xml(image, image_name, bboxes, path_out, show, root):
    ...
    image_path_out = path_out + "{}.jpg".format(image_num)
    ...
    for bbox in bboxes:
        ...
        ET.SubElement(obj, "bbox").text = "{} {} {} {}".format(xmin, ymin, xmax, ymax)
        object_num += 1

        if show:
            img = cv2.rectangle(image, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (128, 0, 255))
            path_out0 = path_out + "gt/"
            cv2.imwrite(path_out0 + "{}.jpg".format(image_num), img)
...
def main():
    ...
    '''
    path_root = './test_re'
    images_path = './img_release/'
    gt_name = './img_release_label_txt.txt'
    #gt_name = './yuncong_no_aug_9323_9320.txt'
    #gt_name = './augfile_yuncong_28786_28790.txt'
    mode = 'train'
    show = True
    if mode == "train":
    ...
    while idx < len(lines):
        ...
        image = cv2.imread("/media/power/f/libface_caffe/yuncong/img_release/00182.jpg")

        image_name_out = image_name.split('/')[-1]
        image_name_out = image_name_out[:-4] + ".jpg"
